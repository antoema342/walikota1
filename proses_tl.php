...
$sql ="INSERT INTO tbl_tl (nama_staf, tgl, skpd_ukpd, tlp, ip_address, pengecekan_perbaikan, p_perbaikanjaringan, p_perangkatjaringan, pemakaian_bphtik, penjelasan_teknis,id_disposisi) values ('$nama_staf', '$tgl', '$skpd_ukpd', '$tlp', '$ip_address', '$pengecekan_perbaikan', '$p_perbaikanjaringan', '$p_perangkatjaringan', '$pemakaian_bphtik', '$penjelasan_teknis','$where')";

$query = "UPDATE tbl_disposisisurat SET status='Selesai' WHERE id_disposisi=$where";

if ($config->query($query) === TRUE) {
    echo "<script>alert('Berhasil Tindak Lanjut')</script>";
echo "<meta http-equiv='refresh' content='1 url=tambah_TL.php'>";
} else {
...
echo "<meta http-equiv='refresh' content='5 url=tambah_TL.php'>";
}
}
