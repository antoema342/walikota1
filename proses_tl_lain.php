...
$sql ="INSERT INTO tbl_tl_lain (nama_staf, tgl, skpd_ukpd, tlp, ip_address, pengecekan_perbaikan, p_perbaikanjaringan, p_perangkatjaringan, pemakaian_bphtik, penjelasan_teknis,id_disposisilain) values ('$nama_staf', '$tgl', '$skpd_ukpd', '$tlp', '$ip_address', '$pengecekan_perbaikan', '$p_perbaikanjaringan', '$p_perangkatjaringan', '$pemakaian_bphtik', '$penjelasan_teknis', '$where')";

$query = "UPDATE tbl_disposisilain SET status='Selesai' WHERE id_disposisilain=$where";

if ($config->query($query) === TRUE) {
    echo "<script>alert('Berhasil Tindak Lanjut')</script>";
echo "<meta http-equiv='refresh' content='1 url=tambah_tl_lain.php'>";
} else {
...
echo "<meta http-equiv='refresh' content='5 url=tambah_tl_lain.php'>";
}
}
