            ...
            if(isset($_REQUEST['act'])){
                ...
                switch ($act) {
                    case 'add':
                        ...
                        break;
                    case 'edit':
                        include "edit_tllain.php";
                        break;
                    case 'ttl':
                        include "tambah_tl_lain.php";
                        break;
                    case 'print':
                    ...
                    if($cdata > $limit ){
                        ...
                        <div class="col m12" id="colres">
                            <table class="bordered" id="tbl">
                                ...
                                $query = mysqli_query($config, "SELECT * FROM tbl_disposisilain where kepada = $where  ORDER by id_disposisilain DESC LIMIT $curr, $limit");
                               if(mysqli_num_rows($query) > 0){
                                    $no = 1;
                                    while($row = mysqli_fetch_array($query)){
                                      echo '
                                      ...
                                      if($_SESSION['id_user'] != $row['id_user'] AND $_SESSION['id_user'] != 1){
                                            ...
                                            echo '<a class="btn small light-green waves-effect waves-light tooltipped" data-position="left" data-tooltip="Pilih EDIT untuk Edit Lanjut " href="?page=tsm1dll&act=edit&id_surat='.$row['id_disposisilain'].'">
                                                <i class="material-icons">description</i> EDIT</a>';}
                                                else{
                                            echo '<a class="btn small light-green waves-effect waves-light tooltipped" data-position="left" data-tooltip="Pilih TL untuk Tindak Lanjut " href="?page=tsm1dll&act=ttl&id_surat='.$row['id_disposisilain'].'">
                                                <i class="material-icons">description</i> TL</a>';}
                                        } else {
                                          echo '<a class="btn small light-green waves-effect waves-light tooltipped" data-position="left" data-tooltip="Pilih TL untuk Tindak Lanjut Surat" href="?page=tsm1dll&act=ttl&id_surat='.$row['id_surat'].'">
                                                    <i class="material-icons">description</i> TL</a>';
                                        } echo ' 
                                                    <td>'.$row['status'].'</td>';
